...
function fresh(room){return {engineVersion:3,started:true,current:0,rolled:false,extraRoll:false,lastDice:[0,0],players:room.players.map((p,i)=>({onlineId:p.id,name:p.name,character:p.character??i,money:1500000,pos:0,skip:0,jail:false,jailRound:0,jailAttempts:0,doublesStreak:0,bankrupt:false,color:i})),titles:{},deck:[],deckPos:0,logs:['Cada jogador deve lançar os dados para decidir quem começa.'],turnNumber:1,choices:[],trades:[],moneyEvents:[],bankFlow:-1500000*room.players.length,initiative:{round:1,eligible:room.players.map((_,i)=>i),rolls:{},complete:false,winner:null}}}
function init(s){s.choices||=[];s.trades||=[];s.turnNumber||=1;s.logs||=[];s.moneyEvents||=[];s.extraRoll=!!s.extraRoll;s.players.forEach(p=>{p.jailRound=p.jail?p.jailRound||1:0;p.jailAttempts=p.jail?p.jailAttempts||0:0;p.doublesStreak=p.doublesStreak||0});if(!s.initiative)s.initiative={round:1,eligible:s.players.map((_,i)=>i),rolls:{},complete:true,winner:s.current};if(s.engineVersion!==3){s.deck=[];s.deckPos=0;s.engineVersion=3}}
...
function sendToJail(s,i,reason='penalidade'){const p=s.players[i];p.pos=10;p.jail=true;p.jailRound=1;p.jailAttempts=0;p.awaitingMovementRoll=false;p.doublesStreak=0;s.rolled=true;s.extraRoll=false;log(s,`${p.name} foi para o lado "Teje preso" (${reason}).`)}
function land(s,i,d){const p=s.players[i],k=p.pos,t=s.titles[k];if(P[k]){if(t&&t.owner!==i){if(p.veto){p.veto=false;log(s,'Veto: aluguel dispensado.')}else transfer(s,i,t.owner,rent(s,k,d[0]+d[1]),'Aluguel de '+P[k][0],true)}}else if([6,11,24,27,32,38].includes(k))draw(s,i);else if(k===8)transfer(s,i,null,200000,'Taxa Municipal',true);else if(k===16)transfer(s,null,i,200000,'Bingo');else if(k===20){p.skip++;log(s,p.name+' ficará uma rodada sem jogar por causa do Jogo do Brasil.')}else if(k===30)sendToJail(s,i,'Foi pego, vá para a Delegacia')}
function movePlayer(s,i,d){const p=s.players[i],old=p.pos;p.awaitingMovementRoll=false;p.pos=(old+d[0]+d[1])%40;if(p.pos<old){if(p.skipSalary){p.skipSalary=false;log(s,p.name+' não recebeu a mixaria (Crise de Reputação).')}else transfer(s,null,i,200000,'Passagem pelo início')}log(s,p.name+' caiu em '+names[p.pos]+'.');land(s,i,d)}
function playRoll(s,i,d){const p=s.players[i];assert(Array.isArray(d)&&d.length===2&&d.every(n=>Number.isInteger(n)&&n>=1&&n<=6),'Dados inválidos.');
 if(p.jail){assert(!s.rolled,'Esta rodada da prisão já terminou.');const max=p.jailRound||1;assert((p.jailAttempts||0)<max,'Suas tentativas desta rodada terminaram.');s.lastDice=d;p.jailAttempts=(p.jailAttempts||0)+1;if(d[0]===d[1]){p.jail=false;p.jailRound=0;p.jailAttempts=0;p.awaitingMovementRoll=true;p.doublesStreak=0;s.rolled=false;s.extraRoll=false;log(s,p.name+' tirou uma dupla e saiu do lado "Teje preso". Deve jogar novamente para se mover.')}else if(p.jailAttempts>=max){s.rolled=true;log(s,`${p.name} não conseguiu uma dupla na ${p.jailRound}ª rodada da prisão.`)}else{s.rolled=false;log(s,`${p.name} não conseguiu uma dupla. Restam ${max-p.jailAttempts} tentativa(s) nesta rodada.`)}return}
 const wasExtraRoll=s.extraRoll;assert(!s.rolled||wasExtraRoll,'Você já jogou os dados.');if(p.skip&&!wasExtraRoll){p.skip--;p.doublesStreak=0;s.extraRoll=false;s.rolled=true;log(s,p.name+' perdeu esta rodada por causa do Jogo do Brasil.');return}s.extraRoll=false;s.rolled=true;s.lastDice=d;const doubles=d[0]===d[1];p.doublesStreak=doubles?(p.doublesStreak||0)+1:0;if(p.doublesStreak>=3){sendToJail(s,i,'três duplas consecutivas');return}movePlayer(s,i,d);if(doubles&&!p.jail){s.extraRoll=true;log(s,p.name+' tirou uma dupla e ganhou uma nova rolagem.')}}
function action(s,i,b){init(s);assert(s.players[i]&&!s.players[i].bankrupt,'Jogador indisponível.');
 if(b.action==='initiative_roll'){const q=s.initiative;assert(!q.complete,'A ordem já foi definida.');assert(q.eligible.includes(i),'Você não participa deste desempate.');assert(q.rolls[i]===undefined,'Você já lançou os dados nesta rodada.');const dice=[randomInt(1,7),randomInt(1,7)];q.rolls[i]={dice,total:dice[0]+dice[1]};log(s,`${s.players[i].name} tirou ${dice[0]} + ${dice[1]} = ${dice[0]+dice[1]} na definição da ordem.`);if(q.eligible.every(j=>q.rolls[j])){const top=Math.max(...q.eligible.map(j=>q.rolls[j].total)),tied=q.eligible.filter(j=>q.rolls[j].total===top);if(tied.length===1){q.complete=true;q.winner=tied[0];s.current=tied[0];s.lastDice=[0,0];log(s,`${s.players[tied[0]].name} começa a partida!`)}else{q.round++;q.eligible=tied;q.rolls={};log(s,`Empate entre ${tied.map(j=>s.players[j].name).join(' e ')}. Eles lançarão novamente.`)}}return}
 assert(s.initiative.complete,'Todos precisam lançar os dados para definir quem começa.');
 ...
 if(b.action==='habeas'){assert(p.habeas>0&&p.jail,'Você não tem uma carta para usar agora.');p.habeas--;p.jail=false;p.jailRound=0;p.jailAttempts=0;p.awaitingMovementRoll=true;s.rolled=false;s.extraRoll=false;log(s,p.name+' usou o Habeas Corpus e deve jogar os dados para se mover.');return}
 if(b.action==='pay_bail'){assert(p.jail&&p.jailRound>=3&&p.jailAttempts>=3,'A fiança só fica disponível após as três tentativas da 3ª rodada.');assert(p.money>=100000,'Dinheiro insuficiente para pagar a fiança.');transfer(s,i,null,100000,'Fiança da Delegacia');p.jail=false;p.jailRound=0;p.jailAttempts=0;p.awaitingMovementRoll=true;s.rolled=false;s.extraRoll=false;log(s,p.name+' pagou a fiança e deve jogar os dados para se mover.');return}
 ...
 if(b.action==='roll'){playRoll(s,i,[randomInt(1,7),randomInt(1,7)]);return}
 if(b.action==='end'){assert(s.rolled,'Jogue os dados primeiro.');assert(!s.extraRoll,'Você ganhou uma nova rolagem por tirar uma dupla.');if(p.jail&&p.jailAttempts>=p.jailRound&&p.jailRound<3){p.jailRound++;p.jailAttempts=0;log(s,`${p.name} terá ${p.jailRound} tentativa(s) na próxima rodada da prisão.`)}p.doublesStreak=0;let j=i;do{j=(j+1)%s.players.length}while(s.players[j].bankrupt&&j!==i);s.current=j;s.rolled=false;s.extraRoll=false;s.lastDice=[0,0];s.turnNumber++;log(s,'Agora é a vez de '+s.players[j].name+'.');return}
...
module.exports={fresh,action,draw,init,resolve,removePlayer,buildable,cost,rent,cardNames,playRoll,sendToJail};
