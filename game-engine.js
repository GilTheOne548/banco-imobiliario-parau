...
function fresh(room){return {engineVersion:2,started:true,current:0,rolled:false,lastDice:[0,0],players:room.players.map((p,i)=>({onlineId:p.id,name:p.name,character:p.character??i,money:1500000,pos:0,skip:0,jail:false,jailTurns:0,bankrupt:false,color:i})),titles:{},deck:[],deckPos:0,logs:['Cada jogador deve lançar os dados para decidir quem começa.'],turnNumber:1,choices:[],trades:[],moneyEvents:[],bankFlow:-1500000*room.players.length,initiative:{round:1,eligible:room.players.map((_,i)=>i),rolls:{},complete:false,winner:null}}}
function init(s){s.choices||=[];s.trades||=[];s.turnNumber||=1;s.logs||=[];s.moneyEvents||=[];if(!s.initiative)s.initiative={round:1,eligible:s.players.map((_,i)=>i),rolls:{},complete:true,winner:s.current};if(s.engineVersion!==2){s.deck=[];s.deckPos=0;s.engineVersion=2}}
